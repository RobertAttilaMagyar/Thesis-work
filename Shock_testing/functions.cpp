...
void Write_Editors(std::ofstream &of, EDITOR *editors, int num_editors)
{
    if (of.is_open())
    {
        ...
        {
            of << i;
            if (i != editors[0].DIMENSION - 1)
                of << ";"; // avoid putting a terminator at the end of each line
        }
        of << "\n";
        ...
        for (auto i = 0; i < num_editors; ++i)
        {
            for (auto j = 0; j < editors[i].DIMENSION; ++j)
            {
                ...
                if (j != editors[i].DIMENSION - 1)
                    of << ";"; // avoid putting a terminator at the end of each line
            }
            if (i != editors[i].DIMENSION - 1)
...
void Write_2dVector(std::ofstream &of, std::vector<std::vector<int>> &dist_mx)
{
    if (of.is_open())
    {
        for (auto i = 0; i < dist_mx.size(); ++i)
        {
            for (auto j = 0; j < dist_mx[i].size(); ++j)
            {
                of << dist_mx[i][j];
                if (j != dist_mx[i].size() - 1)
                    of << ",";
            }
            of << "\n";
        }
...
void Pair_ed_to_art(std::vector<EDITOR> editors, std::vector<ARTICLE> articles, std::vector<int> &art_ind)
{
    ...
    for (auto i = 0; i < num_editors; ++i)
        norm_d[i] = new float[num_articles];
    for (auto i = 0; i < num_editors; ++i)
    {
        for (auto j = 0; j < num_articles; ++j)
        {
            if (row_sums[i] == 0)
                ...
                norm_d[i][j] = (float)d[i][j] / row_sums[i];
        }
    }
    for (auto i = 0; i < num_editors; ++i)
        delete[] d[i];
...
/*  The "game" cycle*/
void Time_Development(std::vector<EDITOR> editors, std::vector<ARTICLE> articles, int Tmax, int Tprime, int Nprime)
{
    int num_editors = editors.size();
    int num_articles = articles.size();
    int j;
    int dim = articles[0].DIMENSION;
    int *NumOfEdits = new int[Tmax];

    // initialize the dist_mx which should be dynamic
    std::vector<std::vector<int>> dist_mx;
    for (auto i = 0; i < num_editors; ++i)
    {
        std::vector<int> new_row;
        dist_mx.push_back(new_row);
        for (auto j = 0; j < num_articles; ++j)
            dist_mx[i].push_back(0);
    }

    float p_newed = 0.1;
    float p_newart = 1 - p_newed;
    std::vector<int> art_ind;
    // Running the process for T time, and counting how many times does the editor i edits article j
    for (int x = 0; x < Tmax; ++x)
    {
        std::cout << "x: " << x << '\n';
        // PROCESSES AT T' when the shock happens and new articles are added to the simulation
        if (x < Tprime)
        {
            float f = (float)rand() / RAND_MAX;
            if (f < p_newed)
            {
                std::cout << "New editor added. "
                          << "f: " << f << "\n";
                EDITOR newed;
                editors.push_back(newed);

                // Incrementing the matrix with new line full of zeros
                std::vector<int> new_row;
                dist_mx.push_back(new_row);
                for (auto j = 0; j < num_articles; ++j)
                    dist_mx[num_editors].push_back(0);
                num_editors += 1;
            }
            else
            {
                std::cout << "New article added. "
                          << "f: " << f << "\n";
                ARTICLE newart;
                articles.push_back(newart);

                // incrementing the matrix with new coloum
                for (auto i = 0; i < num_editors; ++i)
                    dist_mx[i].push_back(0);
                num_articles += 1;
            }
        ...
        for (auto i = 0; i < num_editors; ++i)
        {
            art_ind.push_back(INT32_MAX);
        }
        Pair_ed_to_art(editors, articles, art_ind);
        for (auto i = 0; i < num_editors; ++i)
        {
            j = art_ind[i];
            if (j < num_articles)
        ...
        if (x == 100 || x == 50 || x == 200 || x == Tprime - 25 || x == Tprime || x == Tprime + 25 || x == Tprime + 50 || x == Tprime + 100)
        {
            std::string name;
            name = "distmx_inprocess_continouschange_p_newed" + to_string(p_newed) + "_Tprime" + to_string(Tprime + 1) + "_currentT" + to_string(x) + ".csv";

            std::cout << "Saved to: " << name << "\n";

            std::ofstream of(name);
            Write_2dVector(of, dist_mx);
            std::cout << "Writing finished\n";
        }
    }
    std::string namenumed;
    namenumed = "numedits_continouschange_p_newed" + to_string(p_newed) + "_Tprime" + to_string(Tprime + 1) + "_maxT" + to_string(Tmax) + ".dat";

    std::cout << "Saved to: " << namenumed << "\n";

    std::ofstream of(namenumed);
    if (of.is_open())
    {
    ...
    namefinal = "distmx_continouschange_p_newed" + to_string(p_newed) + "_Tprime" + to_string(Tprime + 1) + "maxT" + to_string(Tmax) + ".csv";

    std::cout << "Saved to: " << namefinal << "\n";

    std::ofstream newof(namefinal);
    Write_2dVector(newof, dist_mx);
